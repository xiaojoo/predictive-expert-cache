from __future__ import annotations

import torch

from predictive_cache.cache import PredictiveExpertCache
from predictive_cache.prefetch import (
    PrefetchEngine,
    PrefetchPipeline,
    PrefetchSource,
    PrefetchTarget,
)
from predictive_cache.routing import (
    QwenMoeRoutingCapture,
    QwenRoutingBridge,
)
from predictive_cache.scheduler import ExpertScheduler

from transformers import Qwen3MoeConfig
from transformers.models.qwen3_moe.modeling_qwen3_moe import (
    Qwen3MoeForCausalLM,
)


def _build_real_qwen() -> Qwen3MoeForCausalLM:
    config = Qwen3MoeConfig(
        num_hidden_layers=2,
        hidden_size=128,
        intermediate_size=256,
        num_local_experts=8,
        num_experts_per_tok=2,
        vocab_size=128,
    )

    model = Qwen3MoeForCausalLM(config)
    model.eval()

    return model


def _capture_routing(
    model: Qwen3MoeForCausalLM,
    capture: QwenMoeRoutingCapture,
    input_ids: torch.Tensor,
    *,
    step: int,
) -> list[int]:
    with torch.no_grad():
        output = capture.capture_forward(
            model,
            input_ids=input_ids,
            step=step,
        )

    assert output.logits.shape[:2] == input_ids.shape

    events = capture.bridge.collector.events()

    assert events

    current_experts = sorted(
        {
            expert_id
            for event in events
            if event.step == step
            for expert_id in event.expert_ids
        }
    )

    assert current_experts

    return current_experts


def test_real_qwen_prediction_gpu_prefetch_lifecycle_suppresses_duplicate():
    """
    6E-6:

        Real Qwen routing
            ↓
        PredictiveExpertCache
            ↓
        prediction
            ↓
        ExpertScheduler
            ↓
        PrefetchPipeline
            ↓
        Admission
            ↓
        PrefetchEngine
            ↓
        NVME → RAM
            ↓
        RAM → GPU
            ↓
        GPU residency metadata
            ↓
        second prediction
            ↓
        scheduler suppresses resident experts
            ↓
        no duplicate prefetch
    """

    model = _build_real_qwen()

    cache = PredictiveExpertCache()

    bridge = QwenRoutingBridge(cache)
    capture = QwenMoeRoutingCapture(bridge)

    input_ids = torch.tensor(
        [
            [10, 11, 12],
            [20, 21, 22],
        ],
        dtype=torch.long,
    )

    routing_history: list[list[int]] = []

    # ---------------------------------------------------------
    # Real Qwen routing history
    # ---------------------------------------------------------

    for step in range(4):
        current_experts = _capture_routing(
            model,
            capture,
            input_ids,
            step=step,
        )

        routing_history.append(current_experts)

    assert len(routing_history) == 4
    assert all(routing_history)

    # ---------------------------------------------------------
    # Prediction
    # ---------------------------------------------------------

    prediction_context = [
        routing_history[-1][0],
    ]

    predictions = cache.predict(
        prediction_context,
        top_k=cache.config.prediction_top_k,
    )

    assert predictions

    predicted_ids = [
        prediction.expert_id
        for prediction in predictions
    ]

    assert predicted_ids
    assert len(predicted_ids) <= cache.config.prediction_top_k
    assert len(predicted_ids) == len(set(predicted_ids))

    assert all(
        0 <= expert_id < model.config.num_local_experts
        for expert_id in predicted_ids
    )

    # IMPORTANT:
    #
    # Predicted experts must NOT already exist in the algorithm-level
    # cache. The cache's LRU represents residency, regardless of the
    # physical location string.
    #
    # NVME is represented by the PrefetchTask source, not by inserting
    # the expert into PredictiveExpertCache before scheduling.
    assert all(
        cache.get(expert_id) is None
        for expert_id in predicted_ids
    )

    assert not (
        set(predicted_ids)
        & set(cache.cached_experts())
    )

    # ---------------------------------------------------------
    # Scheduler
    # ---------------------------------------------------------

    scheduler = ExpertScheduler(cache)

    first_requests = scheduler.plan_prefetch(
        prediction_context,
    )

    first_request_ids = {
        request.expert_id
        for request in first_requests
    }

    assert first_request_ids == set(predicted_ids)

    # ---------------------------------------------------------
    # NVME → RAM storage simulation
    #
    # The physical storage tier is intentionally kept outside the
    # algorithm-level PredictiveExpertCache.
    # ---------------------------------------------------------

    ram_storage: dict[int, torch.Tensor] = {}

    def nvme_to_ram(task):
        tensor = torch.empty(
            256,
            dtype=torch.float32,
        )

        ram_storage[task.expert_id] = tensor

    nvme_engine = PrefetchEngine(
        nvme_to_ram,
        num_workers=1,
    )

    nvme_pipeline = PrefetchPipeline(
        scheduler=scheduler,
        engine=nvme_engine,
        source=PrefetchSource.NVME,
        target=PrefetchTarget.RAM,
    )

    # ---------------------------------------------------------
    # RAM → GPU storage simulation
    # ---------------------------------------------------------

    gpu_storage: dict[int, torch.Tensor] = {}

    def ram_to_gpu(task):
        tensor = ram_storage[task.expert_id]

        gpu_tensor = tensor.to(
            "cuda",
            non_blocking=False,
        )

        gpu_storage[task.expert_id] = gpu_tensor

    gpu_engine = PrefetchEngine(
        ram_to_gpu,
        num_workers=1,
    )

    try:
        # =====================================================
        # Cycle 1
        #
        # prediction
        #   → scheduler
        #   → pipeline
        #   → admission
        #   → NVME → RAM
        # =====================================================

        first_result = nvme_pipeline.process(
            prediction_context,
        )

        assert first_result.current_experts == (
            prediction_context
        )

        first_scheduled_ids = {
            request.expert_id
            for request in first_result.scheduled_requests
        }

        assert first_scheduled_ids == set(predicted_ids)

        first_submitted_ids = {
            task.expert_id
            for task in first_result.submitted_tasks
        }

        assert first_submitted_ids == set(predicted_ids)

        assert all(
            decision.admitted
            for decision in first_result.admission_decisions
        )

        assert (
            first_result.admission_stats.accepted
            == len(predicted_ids)
        )

        assert (
            first_result.admission_stats.rejected
            == 0
        )

        # PrefetchEngine is asynchronous.
        nvme_pipeline.stop(wait=True)

        # NVME → RAM completed.
        assert set(ram_storage) == set(
            predicted_ids
        )

        # =====================================================
        # Cycle 1 continued:
        #
        # RAM → GPU
        # =====================================================

        for expert_id in predicted_ids:
            class TransferTask:
                pass

            task = TransferTask()
            task.expert_id = expert_id

            assert expert_id in ram_storage

            ram_to_gpu(task)

        assert set(gpu_storage) == set(
            predicted_ids
        )

        assert all(
            tensor.is_cuda
            for tensor in gpu_storage.values()
        )

        # =====================================================
        # Synchronize algorithm-level residency metadata.
        #
        # THIS is the point where cache.insert() is correct.
        # =====================================================

        for expert_id in predicted_ids:
            cache.insert(
                expert_id=expert_id,
                size_bytes=1024,
                location="gpu",
            )

        cached_experts = set(
            cache.cached_experts()
        )

        assert set(predicted_ids).issubset(
            cached_experts
        )

        for expert_id in predicted_ids:
            entry = cache.get(expert_id)

            assert entry is not None
            assert entry.location == "gpu"

        # =====================================================
        # Cycle 2
        #
        # Prediction still exists, but all predicted experts
        # are now GPU-resident.
        # =====================================================

        second_predictions = cache.predict(
            prediction_context,
            top_k=cache.config.prediction_top_k,
        )

        assert second_predictions

        second_predicted_ids = {
            prediction.expert_id
            for prediction in second_predictions
        }

        assert second_predicted_ids

        # All second-cycle predicted experts that are already
        # resident must be suppressed.
        second_requests = scheduler.plan_prefetch(
            prediction_context,
        )

        second_request_ids = {
            request.expert_id
            for request in second_requests
        }

        assert second_request_ids.isdisjoint(
            cached_experts
        )

        # In the expected lifecycle, all predicted experts from
        # the second prediction are already GPU resident.
        if second_predicted_ids.issubset(
            cached_experts
        ):
            assert second_request_ids == set()

        # No duplicate GPU transfer occurred.
        assert set(gpu_storage) == set(
            predicted_ids
        )

        for expert_id in predicted_ids:
            assert expert_id in gpu_storage
            assert gpu_storage[expert_id].is_cuda

    finally:
        nvme_pipeline.stop(wait=True)
        gpu_engine.stop(wait=True)