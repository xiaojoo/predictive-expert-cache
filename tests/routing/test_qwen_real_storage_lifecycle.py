...
def test_real_qwen_nvme_ram_gpu_ram():
    ...
    # NVME -> RAM
    #
    # Qwen routing provides the current expert context.
    # ExpertScheduler independently predicts experts to prefetch.
    # These two sets are intentionally allowed to differ.
    # =========================================================
    ...
    # Scheduler prediction must select valid experts, but does not
    # have to equal the experts actually selected by the current
    # Qwen routing step.
    assert set(ram_experts).issubset(
    ...
    try:
        ...
        submitted_gpu_experts = {
            task.expert_id
            for task in gpu_result.submitted_tasks
        }

        assert submitted_gpu_experts == set(ram_experts)
    ...
    for expert_id in ram_experts:
        ...
        assert gpu_record.location == ExpertLocation.RAM

        assert ram_record.payload.device.type == "cpu"
        assert gpu_record.payload.device.type == "cpu"
